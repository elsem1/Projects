            ...
            <div class="flex-grow">
                <div class="section">
                    <div class="form-box">
                        ...
                        <form id="login" class="input-group form-hidden" method="POST" action="{{ route('login') }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="checkbox-wrapper">
                                <input type="checkbox" class="check-box" id="remember-password">
                                <label for="remember-password">Remember Password</label>
                            </div>
                            <button type="submit" class="submit-btn">Log in</button>
                            @guest
                            <p class="small-text mt-2 text-right"><a onclick="forgotPassword()"class="hover:underline">Forgot Password?</a></p>
                            @endguest
                        </form>

                        <form id="register" class="input-group form-hidden" method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
                            @csrf
                        ...
                        @endif
                        <form id="forgot-password" class="input-group form-hidden" method="POST" action="{{ route('password.email') }}" enctype="multipart/form-data">
                            @csrf
