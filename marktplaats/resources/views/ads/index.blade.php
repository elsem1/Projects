        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="ads-section p-6 bg-white shadow-md rounded-lg lg:col-span-2">                
                ...
                <form action="{{ route('ads.index') }}" method="GET">
                    <input type="text" name="query" class="mb-5 0" placeholder="Search ads.." value="{{ old('query', request()->input('query')) }}">
                    <button type="submit">Search</button>
                </form>

                <!-- Dropdown menu dat alle categorieen laat zien -->
                <div class="dropdown" style="float:right;">
                    <button class="dropbtn">Categories</button>
                    <div class="dropdown-content">
                        <hr style="margin: 5px 0; border: 1px solid #ccc;"> 
                        @foreach ($categories as $category)
                            <a href="{{ route('ads.index', ['category_id' => $category->id]) }}">{{ $category->name }}</a>
                        @endforeach
                    </div>
                </div>
                    ...
                    @foreach ($ads as $ad)
                        <li class="p-4 border rounded-lg bg-gray-100 hover:bg-gray-200 transition duration-200">
                            <div class="flex items-center">                               
                                <div class="w-1/4">
                                    <!-- Weergeeft een random afbeelding geladen door de AppServiceProvider -->
                                    <x-ad-img  
                                        ...
                                        <h6>
                                            ...
                                            <span>
                                                ...
                                                @foreach ($ad->categories as $category)
                                                    <a href="{{ route('ads.index', $category->id) }}" class="text-blue-400 hover:underline">{{ $category->name }}</a>
                                                    @if (!$loop->last), @endif 
                                                @endforeach
                                            </span>
                                        @endif
