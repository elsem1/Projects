<div class="w-full md:w-3/4">
                ...
                <li class="p-4 border rounded-lg bg-gray-100 hover:bg-gray-200 transition duration-200">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-sm text-gray-600">By <span class="font-semibold">{{ $bid->user->name }}</span></p>
                        </div>
    ...
    <form id="bid-form" action="{{ route('ads.bids.store', $ad->id) }}" method="POST">
        ...
        <div class="mt-20 mb-3">
            <input type="number" min="0.00" max="10000.00" step="0.01" name="bid" class="fs-6 form-control w-full px-4 py-2 rounded-lg bg-neutral-800 text-slate-200 border border-slate-700"  placeholder="Bid an amount in €"></input>
        </div>
        <div>
            <button type="button" class="mt-6 bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 transition duration-200 btn-bid">
                Place a Bid
            </button>
        </div>
    </form>   
